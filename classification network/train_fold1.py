...
def trainer(gpu, ngpus_per_node, args):
    ...
    # ce_loss = FocalLoss(class_num=args.num_classes, alpha=0.25)
    # sm = torch.nn.Softmax()
    ...
    # for binary classification with under/over-sampling imbalanced data
    # weights = [1/0.6176, 1/0.3824]    
    # class_weights = torch.FloatTensor(weights).cuda()
    # ce_loss = torch.nn.BCEWithLogitsLoss(weight=class_weights)    
    # sm = torch.nn.Sigmoid()
    
    tr_train = Train_Data()
    # tr_train = Train_All_Data()
